...
# 🌐 Language selector
language = st.sidebar.selectbox(msgs["lang_label"]["English"], ["English", "Español"])
...
# 📍 Haversine distance for filtering map points
...
def fetch_water_sources():
    try:
        ...
        st.error(f"Error fetching water sources: {e}")
...
if page == msgs["map"][language]:
    ...
    if df.empty:
        ...
        if filtered.empty:
            ...
            deck = pdk.Deck(layers=[layer], initial_view_state=view, tooltip={"text": "{name}"})
